...
buckets balanced_bucket_recv(const size_t n_buckets, const size_t size, const size_t proc_units, const size_t proc_unit, const size_t rank_size) {
    MPI_Status status;
...
void balanced_bucket_send(const buckets* b, const size_t proc_units) {
    MPI_Request r;
    int last_bucket = 0;
...
int main(int argc, char** argv) {
    ...
    int root = size_rank > n_buckets ? n_buckets : 0;
#ifdef NDEBUG
...
#endif
    int* tmp_stats = rank == root ? stats_as_arr(&stats) : malloc(sizeof(int) * 4);
    MPI_Bcast(tmp_stats, 4, MPI_INT, root, MPI_COMM_WORLD);
    ...
    dyn_arr to_process = recive_dyn_arr(root, MPI_ANY_TAG, stats.buf_size, &status);
    //Build buckets
#ifdef NDEBUG
    ...
    balanced_bucket_send(&b, proc_units);
#ifdef NDEBUG
    fprintf(stderr, "%d: --Send buckets\n", getpid());
#endif
    if(rank < proc_units) {
        //Recv buckets
        buckets r = balanced_bucket_recv(n_buckets, stats.buf_size, proc_units, rank, size_rank);
        //Sort Buckets
        buckets_sort(&r);
        //Join to arr
        dyn_arr res = buckets_to_dyn_arr(&r);
        send_dyn_arr(&res, root, 0);
    }
